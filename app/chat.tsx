...
export default function Chat() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            // ⭐ NOVO: Dodan error handling za getMessages
            ...
            if (msgResult.success) {
                ...
                // ⭐ Postavi error da korisnik vidi zašto ne radi
    ...
    const CountdownTimer = ({ expiresAt }: { expiresAt: string }) => {
        ...
        return (
            <View
                style={[styles.countdownContainer, { borderColor: urgencyColor + '40' }]}
            >
    ...
    const renderBookingCard = (msg: ChatMessage, booking: BookingDetails) => {
        ...
        return (
            <View style={styles.bookingCard}>
                ...
                <View
                    style={[styles.bookingStatusBadge, { backgroundColor: cfg.bg }]}
                >
                ...
                {/* CONFIRMED - PIN */}
                {booking.status === 'confirmed' && booking.pickupPin && (
                    <View style={styles.bookingPinBox}>
                        ...
                        <Text style={styles.bookingPinSubtitle}>
                            {isRenter
                                ? t('chat', 'showPinToOwner')
                                : `${t('chat', 'sharePinWith')} ${booking.renterName} ${t(
                                    'chat',
                                    'atPickup'
                                )}`}
                        </Text>
                        <View style={styles.bookingPinDisplay}>
                            <Text style={styles.bookingPinCode}>{booking.pickupPin}</Text>
                        </View>
                        {isRenter && (
                            <TouchableOpacity
                ...
                {/* CONFIRMED - No PIN yet */}
                {booking.status === 'confirmed' && !booking.pickupPin && isOwner && (
                    <View style={styles.bookingWaitingBox}>
                        <Ionicons name="key-outline" size={20} color={colors.primary} />
                        <Text style={styles.bookingWaitingText}>
                            PIN will be generated automatically. Share it with the renter at
                            pickup.
                        </Text>
                    </View>
                )}
